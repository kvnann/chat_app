...
import { getSavedUserData } from "./handlers";
...
// export const newAnonymTokenKey = async()=>{
//     await AsyncStorage.setItem("anonymToken", "");
//     return await axios.get(`${backend_host}/auth?new=true`,{
//         headers:{
...
export const sortArrayByTime = (array) => {
    ...
    messageArray.sort((a, b) => {
        const timeA = new Date(a.timeSentSent);
        const timeB = new Date(b.timeSentSent);
        return timeA - timeB;
    });
...
export const openChat = (tag, navigation) => {
    const chatData = chat_data.find(chat => {
        return chat.participants.includes(getSavedUserData().tag) &&
               chat.participants.includes(tag) &&
